...
def wait_for_prompt(port, prompt_string):
    prompt_expected = b'\r\n' + prompt_string
    prompt_buffer = b''
    response_buffer = b''
    time_timeout = 10.0
...
def write_control(port, prompt_string, control):
    ...
    port.write(control)
    time.sleep(0.1)
    response_full = wait_for_prompt(port, prompt_string)
...
def write_command(port, prompt_string, command):
    port.reset_output_buffer()
    port.reset_input_buffer()
...
if (__name__ == "__main__"):
    ...
    parser.add_argument("-t", "--tty", help="Hub device path.\n  Linux wired is: /dev/ttyACM0\n Linux bluetooth is: /dev/rfcomm0\n", default="/dev/ttyACM0")
    parser.add_argument("-r", "--reboot", help="Reboot the hub before connecting\n", action='store_true')
    parser.add_argument("-o", "--output", help="Output file to used for responses from the hub.\n", default="")
    ...
    port = serial.Serial(arguments.tty, 115200, timeout = 0.1)
    ...
    response = write_control(port, b'=== ', b'\x05')
    ...
    response = write_control(port, b'>>> ', b'\x04')
    
    if (arguments.output != ""):
        print("Saving script output to file...")
        output_file = open(arguments.output, 'wb') 
